...
const RFQ: React.FC = () => {
  ...
  const [filteredBuyerOptions, setFilteredBuyerOptions] = useState<
    IComboBoxOption[]
  >([]);
  const [selectedBuyerValue, setSelectedBuyerValue] = useState<
    string | undefined
  >();
  ...
  const applyFilters = (): void => {
    ...
    queryRFQs(filters).then(
      (_) => _,
      (_) => _
    ); // 调用 queryRFQs 方法传递过滤参数
  };
    ...
    return (
        <Stack tokens={{childrenGap: 20}} styles={{
          ...
          <DatePicker
            ...
            value={
              searchConditions.RFQReleaseDateFrom
                ? new Date(searchConditions.RFQReleaseDateFrom)
                : undefined
            }
            onSelectDate={(date) => {
              if (date) {
          ...
          <DatePicker
            ...
            value={
              searchConditions.RFQReleaseDateTo
                ? new Date(searchConditions.RFQReleaseDateTo)
                : undefined
            }
            onSelectDate={(date) => {
              if (date) {
          ...
          <DatePicker
            ...
            value={
              searchConditions.RFQDueDateFrom
                ? new Date(searchConditions.RFQDueDateFrom)
                : undefined
            }
            onSelectDate={(date) => {
              if (date) {
          ...
          <DatePicker
            ...
            value={
              searchConditions.RFQDueDateTo
                ? new Date(searchConditions.RFQDueDateTo)
                : undefined
            }
            onSelectDate={(date) => {
              if (date) {
