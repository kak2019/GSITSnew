...
import { ISupplierHostBuyerMapping } from "../../../../model/user";
...
const PriceChangeRequest: React.FC = () => {
  ...
  const [filteredHostBuyerOptions, setFilteredHostBuyerOptions] = useState<
    IComboBoxOption[]
  >([]);
  const [selectedHostBuyerValue, setSelectedHostBuyerValue] = useState<
    string | undefined
  >();
  ...
  // };
  const [hostBuyerStatus, setHostBuyerStatus] = useState<string>("Inactive");
  const { getUserType, getSupplierHostBuyerMapping } = useUser();
  // const [, supplierId, , getSupplierId] = useUsers();
    const [, ,supplierId , , , getSupplierId, , ,] = useUDGSUser();
  let userEmail = "";
   const [supplierhostbuyermappingdetail, setSupplierHostBuyerMappingDetail] = useState<ISupplierHostBuyerMapping | undefined>();
// useState({
//     PARMANbr: "",
//     PARMANm: "",
//     SupplierHostPorg: "",
...
//   });
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(priceChangeRequestList.length / itemsPerPage)
  );
  ...
  const columns: IColumn[] = [
    ...
    {
      key: "HostBuyerName",
      name: t("Host Buyer Name"),
      fieldName: "HostBuyerName",
      minWidth: 150,
      isResizable: true,
    },
    {
    ...
    },
    {
      key: "Status",
      name: t("Status"),
      fieldName: "SupplierRequestStatus",
      minWidth: 100,
      isResizable: true,
    },
    {
      key: "SupplierRequestedDate",
      name: t("Supplier Requested Date"),
  ...
  const fetchBuyerdropdown = async (
    input: string
  ...
  const handleHostBuyerSelectionChange = (
  ...
  const fetchHostBuyerStatus = async (buyerCode: string): Promise<string> => {
    try {
      const client = getAADClient();
      const functionUrl = `${CONST.azureFunctionBaseUrl}/api/GetGPSUser/${buyerCode}`;
      const response = await client.get(functionUrl, AadHttpClient.configurations.v1);
  
      if (response.ok) {
        const result = await response.json();
        console.log("result",result);
        return result[0].status || "Inactive"; // 默认状态为 Inactive
        
      } else {
        console.error("Failed to fetch Host Buyer status");
        return "Inactive";
      }
    } catch (error) {
      console.error("Error fetching Host Buyer status:", error);
      return "Inactive";
    }
  };
    ...
    const initializeFilters = async (): Promise<void> => {
      try {
        ...
        if (usertype === "Guest") {
          ...
          setSearchConditions((prev) => ({
            ...prev,
            ParmaAccurate: defaultConditions.ParmaAccurate,
          }));
  
          try {
            const supplierhostbuyermappinginfo = await getSupplierHostBuyerMapping(supplierId);
            console.log("supplierdetail", supplierhostbuyermappinginfo);
            setSupplierHostBuyerMappingDetail(supplierhostbuyermappinginfo);
  
            if (supplierhostbuyermappinginfo?.SupplierHostPorg && supplierhostbuyermappinginfo?.SupplierHostCd) {
              const buyerCode = `${supplierhostbuyermappinginfo.SupplierHostPorg}-${supplierhostbuyermappinginfo.SupplierHostCd}`;
              console.log("buyercode", buyerCode);
              const status = await fetchHostBuyerStatus(buyerCode);
              setHostBuyerStatus(status); // set Host Buyer status
              console.log("Host Buyer Status:", status);
            }
          } catch (error) {
            console.warn("get Supplier Host Buyer Mapping failed, continue...", error);
          }
        }
        ...
        await getSupplierRequestList(defaultConditions)
          .then(
            (_) => _,
            (_) => _
          )
  ...
  const applyFilters = (): void => {
    const filters = {
      ...
      expectedeffectivedatefrom: searchConditions.ExpectedEffectiveDateFrom
        ? new Date(
            new Date(searchConditions.ExpectedEffectiveDateFrom).getTime()
          )
        : "",
    ...
    getSupplierRequestList(filters)
      .then(
        (_) => _,
        (_) => _
      )
      .catch((e) => console.log(e)); // 调用 queryRFQs 方法传递过滤参数
  };
  ...
  const filterasHostBuyer = (): void => {
    ...
    console.log("loginuserdetail", loginuserdetail);
    getSupplierRequestList(filtercondition)
      .then(
        (_) => _,
        (_) => _
      )
  ...
  const filterasResponsibleBuyer = (): void => {
    const filtercondition = {
      ResponsibleBuyer: loginuserdetail,
    };
    console.log("filterasHostBuyer", filtercondition);
    console.log("loginuserdetail", loginuserdetail);
    getSupplierRequestList(filtercondition)
  ...
  return (
    ...
    >
      ...
      {isSearchVisible && (
        ...
        >
          ...
          <Dropdown
            ...
            selectedKeys={searchConditions.SupplierRequestStatus}
            options={StatusOptions}
            onChange={(e, option) =>{
              if(option){
                
                const selectedKeys = searchConditions.SupplierRequestStatus ;
                const updatedSelection: string[] = option.selected ? [...selectedKeys, option.key.toString()]      //  选中：加入选项
                : selectedKeys.filter((key) => key !== option.key.toString()); //  取消：移除选项
        
              console.log("selectedstatus", updatedSelection);
        
              handleSearchChange("SupplierRequestStatus", updatedSelection);
            }
          }}
          multiSelect //  启用多选
        />
          ...
          {userType === "Member" && (
            <TextField
              ...
              onChange={(e, newValue) =>
                handleSearchChange("ParmaBlur", newValue || "")
              }
              styles={theme.TextfieldStyles}
            />
          )}
          ...
          {userType === "Member" && (
            <>
              <PrimaryButton
                text={t("Show my request as Host Buyer")}
                styles={{
                  ...theme.buttonStyles,
              ...
              <PrimaryButton
                text={t("Show my request as Responsible Buyer")}
                styles={{
                  ...theme.buttonStyles,
                  root: { ...buttonStyles.root, width: "100%" },
                }}
                style={{ gridRow: "3", gridColumn: "4", justifySelf: "end" }}
                onClick={filterasResponsibleBuyer}
              />
            </>
          )}
        </Stack>
      ...
      {/* {false ? (
        ...
        <Stack>
          ...
          <DetailsList
            ...
            columns={columns}
            selectionMode={
              userType === "Member" ? SelectionMode.single : SelectionMode.none
            }
      ...
      {userType === "Member" && (
        <>
          <Stack horizontal tokens={{ childrenGap: 10 }}>
            <DefaultButton
              text={t("Forward to Responsible Buyers")}
              onClick={() => setIsForwardDialogOpen(true)}
              styles={{
                ...theme.buttonStyles,
                root: { ...buttonStyles.root, width: "170px" },
              }}
              disabled={selectedItems.length ===0 || (selectedItems[0].SupplierRequestStatus !== "New" && selectedItems[0].SupplierRequestStatus !== "In Progress" ) || selectedItems[0].HostBuyer !== loginuserdetail }
            />

            <DefaultButton
              text={t("Return Reuqest")}
              onClick={() => setIsReturnDialogOpen(true)}
              styles={{
                ...theme.buttonStyles,
                root: { ...buttonStyles.root, width: "170px" },
              }}
              disabled={selectedItems.length ===0 || selectedItems[0].SupplierRequestStatus !== "New" || selectedItems[0].HostBuyer !== loginuserdetail}
            />
          </Stack>
        </>
      )}
      <CreatePriceChangeRequest
        ...
        parmaID={supplierId}
        hostbuyername={supplierhostbuyermappingdetail?.SupplierHostName || ""}
        hostbuyer={`${supplierhostbuyermappingdetail?.SupplierHostPorg} ${supplierhostbuyermappingdetail?.SupplierHostCd}`}
        hostBuyerStatus={hostBuyerStatus}
      ...
      <ForwardDialog
        ...
        RequestIDRef={selectedItems[0]?.ID || ""}
        SelectedItemStatus={selectedItems[0]?.SupplierRequestStatus || ""}
      />
      <ReturnDialog
        isOpen={isReturnDialogOpen}
        onDismiss={() => setIsReturnDialogOpen(false)}
        onConfirm={handleReturnConfirm}
        RequestID={selectedItems[0]?.ID || ""}
        HostBuyerName={selectedItems[0]?.HostBuyerName || ""}
      />
    </Stack>
  );
};
